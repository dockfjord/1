...
config_after_install() {
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        ...
        echo -e "${red}已取消设定，默认tgtg...${plain}"
        /usr/local/x-ui/x-ui setting -username tgtg -password zxc6996. -port 56996 -webBasePath tgtg
    fi
}
...
install_x-ui() {
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://gitlab.com/lv_vl/1/-/raw/1/3xui.sh
